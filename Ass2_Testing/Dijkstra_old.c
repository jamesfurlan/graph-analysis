...
ShortestPaths dijkstra(Graph g, Vertex src) {
    ...
    while (set[dij.numNodes] < dij.numNodes && !PQIsEmpty(toDo)) {
        ...
        for (AdjList outPaths = GraphOutIncident(g, v.key); outPaths != NULL; outPaths = outPaths->next) {
            ...
            newPred->v = v.key;
            newPred->next = NULL;
            if (dij.dist[v.key] + outPaths->weight < dij.dist[outPaths->v] || dij.dist[outPaths->v] == 0) {
            ...
            } else if (dij.dist[v.key] + outPaths->weight == dij.dist[outPaths->v] || dij.dist[outPaths->v] == 0) {
                PredNode* predCrawl = dij.pred[outPaths->v];
